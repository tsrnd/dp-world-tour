...
class JwtAuthenticationMiddleware(MiddlewareMixin):
    list_ignore_auth_iwt = ['/api/user/register', '/api/user/register/', '/api/user/token', '/api/user/token/']
    ...
    def __call__(self, request):
        ...
        # Check request path 
        if request.path_info.startswith('/api') and request.path_info not in self.list_ignore_auth_iwt:
            try:
